...
struct BungeePy
{
          ...
          sample_rate(sample_rate) // 存储采样率，用于分块
    {
        ...
        // 应用预填充缩放系数，增加预填充帧数
        inputSamplesForPreroll *= preroll_scale; 
        ...
        while (offset < frames)
        {
            ...
            else
            {
                ...
                if (debug_enabled)
                {
                    ...
                    { // First chunk but no padding applied (or frames == 0)
            ...
            if (totalInputFramesForStream == 0 && out_cap < 1024)
                out_cap = 1024; // Ensure some capacity even if no input
            ...
            if (effectiveOutFrames < 0)
                effectiveOutFrames = 0;
...
private:
    ...
    double pitch_param;
    int channels;
    int initialSilenceOutputSamples;
    bool processedFirstChunk = false;
    bool debug_enabled = false;
    double preroll_scale;
    int sample_rate;
};
...
PYBIND11_MODULE(bungee, m)
{
    py::class_<BungeePy>(m, "Bungee")
        ...
        .def("set_debug", &BungeePy::setDebug, py::arg("enable"), "Enable or disable debug messages.")
        .def("get_debug", &BungeePy::getDebug, "Get current debug status.");
}
